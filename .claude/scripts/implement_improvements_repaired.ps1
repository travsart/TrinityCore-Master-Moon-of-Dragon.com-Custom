...
'@
    ...
    $depCheckScript | Out-File -FilePath "$ClaudeDir\scripts\dependency_scanner.py" -Encoding UTF8
...
'@
    ...
    $dashboardHtml | Out-File -FilePath "$ClaudeDir\monitoring\dashboard.html" -Encoding UTF8
...
if ($QuickWins) {
    ...
    # Create pre-commit hook (Windows compatible)
    $preCommitHookBat = '@echo off
    ...
    try {
        ...
        if (-not $taskExists) {
            $action = New-ScheduledTaskAction `
                -Execute "cmd.exe" `
                -Argument "/c `"$ClaudeDir\scripts\run_daily_checks.bat`" --type morning"
            
            $trigger = New-ScheduledTaskTrigger -Once -At (Get-Date).AddHours(1) -RepetitionInterval (New-TimeSpan -Hours 1)
            
            Register-ScheduledTask `
                -TaskName "PlayerBot_HourlyCheck" `
                -Action $action `
                -Trigger $trigger `
                -Description "Hourly health check for PlayerBot" | Out-Null
