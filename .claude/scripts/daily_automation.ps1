...
function Start-SecurityScan {
    ...
    if ($result.vulnerabilities -and $result.vulnerabilities.critical -gt 0) {
...
function Start-PerformanceAnalysis {
    ...
    # Fixed: Added null check for performance_score
    if ($result.performance_score -and $result.performance_score -lt 70) {
...
function Start-CompleteReview {
    ...
    if (Test-Path $pythonScript) {
        ...
        # Fixed: Added try-catch for Python execution and parameter validation
        try {
...
"@
    ...
    # Fixed: Variable shadowing issue by using different variable name
    foreach ($resultKey in $Results.Keys) {
        $result = $Results[$resultKey]
        
        # Fixed: Handle different data types - ensure $result is a hashtable
        if ($result -is [string]) {
            # Convert string to hashtable
            $result = @{ status = "info"; message = $result }
            ...
            # Convert other types to hashtable
            $result = @{ status = "unknown"; message = $result.ToString() }
        }
        ...
        # Fixed: Variable shadowing issue - changed inner loop variable
        foreach ($propertyKey in $result.Keys) {
            if ($propertyKey -match "score|rating|percentage") {
                $html += "<p>$propertyKey`: $($result[$propertyKey])</p>"
            }
        }
...
"@
    ...
    # Fixed: Added try-catch for file operations
    try {
...
function Start-DailyAutomation {
    ...
    foreach ($result in $allResults.Values) {
        # Fixed: Added null check and type validation for result object
        if ($result) {
            ...
            if ($result -is [string]) {
                ...
                if ($result.ContainsKey("status") -and ($result.status -eq "critical" -or $result.status -eq "error")) {
                    $criticalIssues++
                }
            }
        }
