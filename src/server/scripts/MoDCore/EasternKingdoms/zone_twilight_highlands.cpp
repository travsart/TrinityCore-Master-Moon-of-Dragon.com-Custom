        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (uiPhase)
            {
                ...
                if (uiTimer <= uiDiff)
                {
                    ...
                    {
                        case 1:
                            if (Creature* summon = me->SummonCreature(NPC_HURP_DERP, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 5s))
                        ...
                        case 4:
                            ...
                            {
                                if (Creature* summon = me->SummonCreature(NPC_TORG_DRAKEFLAYER, SpawnPosition[1], TEMPSUMMON_CORPSE_DESPAWN, 5s))
                                    SummonGUID = summon->GetGUID();
                            }
                            else
                            if (player->GetTeamId() == TEAM_HORDE)
                            {
                                if (Creature* summon = me->SummonCreature(NPC_SULLY_KNEECAPPER, SpawnPosition[1], TEMPSUMMON_CORPSE_DESPAWN, 5s))
                                    SummonGUID = summon->GetGUID();
                            }
                            uiTimer = 3000;
                            uiPhase = 0;
                            break;
                        case 6:
                            if (Creature* summon = me->SummonCreature(NPC_CADAVER_COLLAGE, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 5s))
                                SummonGUID = summon->GetGUID();
                            uiPhase = 7;
                            uiTimer = 4000;
                            break;
                         case 7:
                            ...
                            break;
                        case 8:
                            if (Creature* summon = me->SummonCreature(NPC_LORD_GEOFFERY_TULVAN, SpawnPosition[2], TEMPSUMMON_CORPSE_DESPAWN, 5s))
                                SummonGUID = summon->GetGUID();
                            uiPhase = 9;
                            uiTimer = 4000;
                            break;
                         case 9:
                            ...
                            break;
                        case 10:
                            if (Creature* summon = me->SummonCreature(NPC_EMBERSCAR_THE_DEVOURER, SpawnPosition[1], TEMPSUMMON_CORPSE_DESPAWN, 5s))
                                SummonGUID = summon->GetGUID();
                            uiPhase = 11;
                            uiTimer = 4000;
                            break;
...
public:
    ...
    struct npc_hurp_derpAI : public ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_INTIMIDATING_ROAR, 10s);
            events.ScheduleEvent(EVENT_OVERHEAD_SMASH, 5s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_INTIMIDATING_ROAR:
                    ...
                    events.ScheduleEvent(EVENT_INTIMIDATING_ROAR, Seconds(urand(15, 25)));
                    break;
                case EVENT_OVERHEAD_SMASH:
                    DoCastVictim(SPELL_OVERHEAD_SMASH);
                    events.ScheduleEvent(EVENT_OVERHEAD_SMASH, Seconds(urand(20, 30)));
                    break;
                case EVENT_WHIRLWIND:
                    ...
                    events.ScheduleEvent(EVENT_WHIRLWIND, Seconds(urand(15, 25)));
                    break;
                }
            }
...
public:
    ...
    struct npc_sully_kneecapperAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_S_CHARGE, 10s);
            events.ScheduleEvent(EVENT_UPPERCUT, 5s);

            me->SummonCreature(NPC_GLOOMWING, SpawnPosition[3], TEMPSUMMON_CORPSE_DESPAWN, 5s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_S_CHARGE:
                    ...
                    events.ScheduleEvent(EVENT_S_CHARGE, Seconds(urand(10, 20)));
                    break;
                case EVENT_UPPERCUT:
                    DoCastVictim(SPELL_UPPERCUT);
                    events.ScheduleEvent(EVENT_UPPERCUT, Seconds(urand(15, 25)));
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_cadaver_collageAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_BELCH, 15s);
            events.ScheduleEvent(EVENT_POISON_CLOUD, 10s);
            events.ScheduleEvent(EVENT_REPULSIVE_KICK, 8s);
        }
       ...
       void DamageTaken(Unit* attacker, uint32& /*damage*/, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if(!inhaled && !me->IsNonMeleeSpellCast(false) && HealthBelowPct(20))
            {
                ...
                events.ScheduleEvent(EVENT_BELOW5, 6s);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BELCH:
                    ...
                    events.ScheduleEvent(EVENT_BELCH, Seconds(urand(15, 20)));
                    break;
                case EVENT_POISON_CLOUD:
                    DoCast(SPELL_POISON_CLOUD);
                    events.ScheduleEvent(EVENT_POISON_CLOUD, Seconds(urand(10, 15)));
                    break;
                case EVENT_REPULSIVE_KICK:
                    DoCastVictim(SPELL_REPULSIVE_KICK);
                    events.ScheduleEvent(EVENT_REPULSIVE_KICK, Seconds(urand(10, 15)));
                    break;
                case EVENT_BELOW5:
                    me->SetUnitFlag2(UNIT_FLAG2_FEIGN_DEATH);
                    me->AddAura(SPELL_CADAVER_HOVER, me);
                    events.ScheduleEvent(EVENT_TAKEOFF, 5200ms);
                    break;
                case EVENT_TAKEOFF:
                    me->SetCanFly(true);
//                    me->SetAnimTier(UnitBytes1_Flags(UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER), true);
                    ...
                    events.ScheduleEvent(EVENT_EXPLODE, 5600ms);
                    break;
                case EVENT_EXPLODE:
                    DoCast(SPELL_PLAGUE_EXPLOSION);
                    me->DespawnOrUnsummon(700ms);
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_lord_geofferyAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_DEATH_BY_PEASANT, 15s);
            events.ScheduleEvent(EVENT_GEOF_UCUT, 5s);
        }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if(!transformed && !me->IsNonMeleeSpellCast(false) && HealthBelowPct(20))
            {
               ...
               events.ScheduleEvent(EVENT_UNDYING_FRENZY, 2s);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_DEATH_BY_PEASANT:
                    ...
                    events.ScheduleEvent(EVENT_DEATH_BY_PEASANT, Seconds(urand(30, 40)));
                    break;
                case EVENT_UNDYING_FRENZY: // execute this event at 20% (needs retail info)
                    DoCastVictim(SPELL_UNDYING_FRENZY);
                    events.ScheduleEvent(EVENT_UNDYING_FRENZY, Seconds(urand(10, 15)));
                    break;
                case EVENT_GEOF_UCUT:
                    DoCastVictim(SPELL_UPPERCUT);
                    events.ScheduleEvent(EVENT_GEOF_UCUT, Seconds(urand(15, 20)));
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_gloomwingAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_GLOOM_BALL, 2s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_GLOOM_BALL:
                    ...
                    events.ScheduleEvent(EVENT_GLOOM_BALL, Seconds(urand(3, 6)));
                    break;
                }
            }
        }
...
void AddSC_MoDCore_twilight_highlands()
{
