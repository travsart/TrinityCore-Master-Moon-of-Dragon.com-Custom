...
// Evade reasons
enum EvadeReasonType
{
    EVADE_REASON_OTHER = 0
};
...
public:
    ...
    {
        npc_stormwind_infantryAI(Creature* creature)
            : ScriptedAI(creature), waitTime(0), wolfTarget(ObjectGuid::Empty) {
        }

        uint32 waitTime;
        ObjectGuid wolfTarget;

        static void GetPositionWithDistInFront(const Unit* unit, float dist, Position& pos)
        {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            else
            {
                ...
                // Hilfsfunktion zum Berechnen einer Position vor einer Einheit
...
struct npc_hogger : public ScriptedAI
{
    ...
    {
        // Initialisiere alle Membervariablen explizit
        _minionsSummoned = false;
        _endingSceneActive = false;
        _alreadyEaten = false;
        _isEating = false;
        _generalHammondGUID.Clear();
        _andromathGUID.Clear();
        _dumasGUID.Clear();
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            ...
            case EVENT_BLOODY_STRIKE:
                 ...
                 [[fallthrough]]; // Markiert absichtlichen Fallthrough
            default:
               break;
            }
        }
