...
public:
    ...
    void OnLogin(Player* player, bool /*firstLogin*/) override
    {
        ...
        {
            if (player->GetClass() != CLASS_DEATH_KNIGHT && player->GetClass() != CLASS_DEMON_HUNTER)
                return;
    ...
    struct npc_anduin_wrynn_107574AI : public EscortAI
    {
        ...
        void OnQuestAccept(Player* player, Quest const* /*quest*/) override
        {
            ...
            if (player->hasQuest(QUEST_THE_ALLIANCE_WAY))
            {
                ...
                // Ersetze alle Vorkommen von aysa->ForcedDespawn(); durch aysa->DespawnOrUnsummon();
                // Beispiel:
                ...
                // Ersetze alle Vorkommen von jojo->ForcedDespawn(); durch jojo->DespawnOrUnsummon();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (EventStart2)
            {
                if (uiPhase_timer <= diff) //quest 46728 dialog_start
                {
                    ...
                    case 32:
                        // Ersetze alle Vorkommen von ForcedDespawn durch DespawnOrUnsummon
                        // Beispiel:
                        if (Creature* jaina = ObjectAccessor::GetCreature(*me, uiJainaGUID))
                        {
                            jaina->CastSpell(jaina, 54219, true);
                            jaina->DespawnOrUnsummon(3000ms); // statt jaina->ForcedDespawn(3000);
                        }
                        JumpToNextStep(1000);
                        break;
