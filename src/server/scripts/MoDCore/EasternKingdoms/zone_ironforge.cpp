...
public:
    ...
    struct npc_mountaineer_watchAI : public ScriptedAI
    {
        npc_mountaineer_watchAI(Creature* creature) : ScriptedAI(creature) { }
    };
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_mountaineer_watchAI(creature);
    }

    bool OnGossipHello(Player* player, Creature* creature)
    {
        if (player->GetQuestStatus(313) == QUEST_STATUS_INCOMPLETE)
            AddGossipItemFor(player, GossipOptionNpc::None, "Captain Tharran wants you to deploy your remote observation bots and withdraw to Kharanos.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF + 1)
        {
            ...
            if (Creature* bot = creature->SummonCreature(41052, creature->GetRandomNearPosition(5.0f), TEMPSUMMON_MANUAL_DESPAWN)) // Summon Remote Observation Bot.
            {
                bot->GetMotionMaster()->MoveRandom(10.0f);
                bot->DespawnOrUnsummon(10s);
            }
