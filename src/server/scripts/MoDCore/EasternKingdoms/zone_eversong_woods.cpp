...
struct npc_apprentice_mirveda : public ScriptedAI
{
    ...
    void OnQuestAccept(Player* player, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_UNEXPECTED_RESULT)
        {
            ...
            me->SummonCreature(NPC_GHARZUL, 8745, -7134.32f, 35.22f, 0, TEMPSUMMON_CORPSE_DESPAWN, 4s);
            me->SummonCreature(NPC_ANGERSHADE, 8745, -7134.32f, 35.22f, 0, TEMPSUMMON_CORPSE_DESPAWN, 4s);
            me->SummonCreature(NPC_ANGERSHADE, 8745, -7134.32f, 35.22f, 0, TEMPSUMMON_CORPSE_DESPAWN, 4s);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (WaveTimer < diff && !Completed && Progress)
        {
            for (uint8 i = 0; i < 3; ++i)
            {
                ...
                me->SummonCreature(NPC_ENRAGED_WRAITH, SpawnLocations[randLocation].x, SpawnLocations[randLocation].y, SpawnLocations[randLocation].z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10s);
            }
...
public:
    ...
    struct master_kelerun_bloodmournAI : public ScriptedAI
    {
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            if (CAST_AI(npc_second_trial_controller::master_kelerun_bloodmournAI, me->AI())->questPhase == 0)
            {
        ...
        void StartEvent()
        {
            ...
            { // no player check, quest can be finished as group, so no complex PlayerGUID/group search code
                for (uint8 i = 0; i < 4; ++i)
                    if (Creature* summoned = DoSpawnCreature(PaladinEntry[i], SpawnPosition[i].x, SpawnPosition[i].y, SpawnPosition[i].z, SpawnPosition[i].o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 3min))
...
public:
    ...
    struct go_second_trialAI : public GameObjectAI
    {
        go_second_trialAI(GameObject* go) : GameObjectAI(go) { }

        bool OnGossipHello(Player* /*player*/) override
        {
            // find spawn :: master_kelerun_bloodmourn
            if (Creature* creature = me->FindNearestCreature(MASTER_KELERUN_BLOODMOURN, 30.0f))
    ...
    {
        return new go_second_trialAI(go);
    }
};
