/*
* This file is part of the DekkCore Team Project. See AUTHORS file for Copyright information
*
...
struct npc_brazie_fertilitize_o_tron_2000 : public ScriptedAI
{
    ...
    {
        if (Creature* vehicle = summoner->ToUnit()->GetVehicleCreatureBase())
    ...
    void AttackStart(Unit* who) override
    {
        ...
        if (Creature* spitterTarget = me->FindNearestCreature(NPC_SPITTER_TARGET, 150.0f, true))
        {
            ...
            pos.SetOrientation(pos.GetAbsoluteAngle(spitterTarget));
...
struct npc_brazie_zombie : public ScriptedAI
{
    ...
    void AttackStart(Unit* who) override
    {
        ...
        Position pos = me->GetPosition();
        pos.SetOrientation(pos.GetAbsoluteAngle(GoalStalkerPositions[_pathIndex]));
...
struct npc_brazie_vehicle_notifier : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        Unit* summoners = summoner->ToUnit();
        if (Creature* vehicle = summoners->GetVehicleCreatureBase())
            if (vehicle->IsAIEnabled())
                vehicle->AI()->JustSummoned(me);
    }
...
class spell_brazie_highlight : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectLaunch += SpellEffectFn(spell_brazie_highlight::SetDestPosition, EFFECT_1, TARGET_DEST_NEARBY_ENTRY);
    }
};
...
class spell_brazie_create_random_seed_sack : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectLaunch += SpellEffectFn(spell_brazie_create_random_seed_sack::SetDestPosition, EFFECT_0, TARGET_DEST_NEARBY_ENTRY);
    }
};
...
class spell_brazie_spit : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_brazie_spit::FilterTargets, EFFECT_ALL, TARGET_UNIT_CONE_CASTER_TO_DEST_ENEMY);
    }
};
