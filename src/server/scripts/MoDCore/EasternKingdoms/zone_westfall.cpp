...
* LINES
* BY
* MOON-OF-DRAGON.COM - World of Warcraft Private Server
* 
...
public:
    ...
    {
        npc_thugAI(Creature* creature)
            : ScriptedAI(creature), SummonTimer(0), Phase(0), bSummoned(false) {
        }
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            {
                const Quest* quest = sObjectMgr->GetQuestTemplate(QUEST_LOUS_PARTING_THOUGHTS);
                if (quest)
                {
                    for (auto const& objective : quest->Objectives)
                    {
                        if (objective.Type == QUEST_OBJECTIVE_MONSTER && !player->IsQuestObjectiveComplete(QUEST_LOUS_PARTING_THOUGHTS, objective.ObjectID))
                        {
                            if (who->IsWithinDistInMap(me, 20.0f) && !bSummoned)
                            {
                                PlayerGUID = who->GetGUID();
                                StartEvent();
                                player->ToPlayer()->KilledMonsterCredit(objective.ObjectID, me->GetGUID());
                            }
                        }
                    }
                }
        ...
        void StartEvent()
        {
            if(!bSummoned)
            {
                for (uint8 i = 0; i < 4; ++i)
                {
                    ...
                    if (Creature* thug = me->SummonCreature(NPC_THUG, ThugPositions[i], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90s))
                    ...
                    if (Player* player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
                    {
                        ...
                        switch (Phase)
                        {
                            ...
                            case 8:
                            {
                                for (uint8 i = 0; i < 4; ++i)
                                {
                                    if (Creature* thug = GetThug(i))
                                    {
                                        ...
                                        thug->SetFaction(14);
                                        thug->SetUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);
                                    }
                                }
                            ...
                            case 9:
                            {
                                for (uint8 i = 0; i < 4; ++i)
                                    if (Creature* thug = GetThug(i))
                                        thug->CanStartAttack(player, true);
...
public:
    ...
    {
        npc_horatioAI(Creature* creature) : ScriptedAI(creature), Phase(0), TextTimer(0), bSummoned(false), bSummoned1(false), bSummoned2(false) {}
        ...
        void StartSummon()
        {
            if (!bSummoned)
            {
                if (Creature* Investigator01 = me->SummonCreature(NPC_INVESTIGATOR2,-9854.414f, 916.481f, 30.100f, 5.3867f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66s))
                {
                    Investigator01GUID = Investigator01->GetGUID();
                    bSummoned1 = true;
                }
                if (Creature* Investigator02 = me->SummonCreature(NPC_INVESTIGATOR2, -9854.252f, 906.651f, 29.833f, 0.730950f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66s))
                {
...
enum edrifter
{
    ...
    QUEST_FEEDING_THE_HUNGRY2 = 26271,
    STEW2 = 42617,
    SPELL_FULL_BELLY2 = 79451,
};
...
public:
    ...
    struct npc_crate_mineAI : public ScriptedAI
    {
        ...
        void PartySummoned()
        {
            if(!bSummoned)
            {
                if (Creature* glubtok1 = me->SummonCreature(42492, -9848.11f, 1395.29f, 37.70f, 0.56f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85s))
                    if (Creature* shadowy1 = me->SummonCreature(42515, -9826.083f, 1406.738f, 36.885f, 3.56f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85s))
                    {
                            ...
                            case 13:
                                {
                                    ...
                                    glubtok1->DespawnOrUnsummon(1s);
                                    shadowy1->CastSpell(shadowy1, 64446, true);
                                    shadowy1->DespawnOrUnsummon(1s);
...
public:
    ...
    {
        npc_shadowy_triggerAI(Creature* creature)
            : ScriptedAI(creature), SummonTimer(0), Phase(0), bSummoned(false), bSummoned1(false), bText(false) {
        }
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (who->ToPlayer()->GetQuestStatus(QUEST_A_VISION_OF_THE_PAST) == QUEST_STATUS_INCOMPLETE)
            {
                if (who->IsWithinDistInMap(me, 2.0f) && !bSummoned)
                {
                    ...
                    who->ToPlayer()->CompleteQuest(QUEST_A_VISION_OF_THE_PAST);
                }
            }
        }
                ...
                if (bSummoned)
                {
                    if (Player* player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
                    {
                        if (Creature* Shadowy2 = ObjectAccessor::GetCreature(*me, Shadowy2GUID))
                        {
                            ...
                            {
                            case 0:
                                me->TextEmote("The rally is about to begin!", nullptr, true);
                                SummonTimer = 1500;
                                Phase++;
                                break;
                            case 1:
                            ...
                            case 6:
                                ...
                                break;
                            case 7:
                                Shadowy2->Yell("The government of Stormwind, of the ALLIANCE, must be made accountable for what it has done to us!", LANG_UNIVERSAL);
                                SummonTimer = 4500;
                                Phase++;
                                break;
                            case 8:
                            ...
                            case 9:
                            {
                                ...
                                me->GetCreatureListWithEntryInGrid(listeners, NPC_LISTENER, 65.0f);
                                if (!listeners.empty() && !bText)
                                {
                                    me->TextEmote("<homeless people applaud and cheer>", nullptr, true);
                                ...
                                // Questabschluss sicherstellen
                                if (player->GetQuestStatus(QUEST_THE_DAWNING_OF_NEW_DAY) == QUEST_STATUS_INCOMPLETE)
                                {
                                    //player->CompleteQuest(QUEST_THE_DAWNING_OF_NEW_DAY);
...
class npc_rise_br : public CreatureScript
{
    ...
    {
        npc_rise_brAI(Creature* creature)
            : ScriptedAI(creature), EventTimer(0), Phase(0), playername('\0'), EventStarted(false) {
        }
        ...
        void OnQuestAccept(Player* player, Quest const* quest) override
        {
            ...
            {
                if (auto ai = dynamic_cast<npc_rise_br::npc_rise_brAI*>(me->AI()))
                {
                    ai->PlayerGUID = player->GetGUID();
                    if (!ai->EventStarted)
                        ai->EventStarted = true;
                }
            }
        }
        ...
        void SummonBrotherHood()
        {
            ...
            if (me->SummonCreature(42755,-10506.12f, 1053.13f, 59.10f, 4.92f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120s))
            if (me->SummonCreature(42753,-10509.368f, 1057.28f, 57.86f, 4.95f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120s))
            {
...
public:
    ...
    {
        npc_defias_blackguardAI(Creature* creature)
            : ScriptedAI(creature),
            timer_DoFire(0), // Reihenfolge angepasst
            timer_DoTheStun(0),
            ThrowTimer(0),
            Defias_DoFire(false),
            Defias_DoTheStun(false)
        {
        }
...
public:
    ...
    {
        npc_horatio_investigateAI(Creature* creature)
            : ScriptedAI(creature), Phase(0), TextTimer(0), bSummonn(false), bSummonn1(false), bSummonn2(false) {
        }
        ...
        void StartSummon()
        {
            if(!bSummonn)
            {
                if (Creature* Investigator04 = me->SummonCreature(NPC_INVESTIGATOR4, -9856.825f, 1279.25f, 40.96f, 3.29f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66s))
                {
                ...
                if (Creature* Home = me->SummonCreature(NPC_HOME, -9862.88f, 1278.52f, 40.80f, 0.43f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66s))
                {
