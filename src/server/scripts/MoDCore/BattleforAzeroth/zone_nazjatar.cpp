    ...
    void MovementInform(uint32 type, uint32 pointID) override
    {
        ...
        if (pointID && !this->portal_prepared)
        {
            ...
            if (this->portal_prepared)
            {
                ...
                {
                    me->SummonGameObject(287164, 1292.530f, -106.669f, -330.105f, 2.41f, QuaternionData(), 0s);
                    Talk(1);
                    me->SetWalk(false);
...
struct boss_wekemara : public BossAI
{
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        case NPC_SPAWN_OF_WEKEMARA:
        {
            ...
            summon->UpdatePosition(pos.GetPositionX(), pos.GetPositionY(), PosZ, summon->GetOrientation());
    ...
    void ExecuteEvent(uint32 eventid) override
    {
        ...
        {
            me->SummonCreature(NPC_WEKEMARA_HANDLER, handler_spawn_at_selector_pos, TEMPSUMMON_TIMED_DESPAWN, 29s);
            events.Repeat(30s);
            break;
        }
...
struct at_wakemara_bioeletric_blast : AreaTriggerAI
{
    ...
    void OnCreate(const Spell* /*spell*/) override
    {
...
struct npc_hidden_treasure : public ScriptedAI
{
    ...
    void Reset() override
    {
        me->SummonGameObject(GO_GLIMMERING_CHEST, me->GetPosition(), QuaternionData(), 1min);
    }
};

//322413
struct go_glimmering_chest : public GameObjectAI
{
    go_glimmering_chest(GameObject* go) : GameObjectAI(go) { }
        ...
        if (unit->IsPlayer() && me->GetDistance2d(unit) < 15.0f)
        {
            if (Player* player = unit->ToPlayer())
            {
                if (player->GetQuestStatus(QUEST_WHERE_THE_ROAD_LEADS_ALLIANCE) == QUEST_STATUS_INCOMPLETE)
                {
                    ...
                    player->GetSceneMgr().PlaySceneByPackageId(SCENE_MEETING_AZSHARA_ALLIANCE, SceneFlag::None);
                }
                if (player->GetQuestStatus(QUEST_STAY_FAST_STAY_LOW_HORDE) == QUEST_STATUS_INCOMPLETE)
                {
                    ...
                    player->GetSceneMgr().PlaySceneByPackageId(SCENE_MEETING_AZSHARA_HORDE, SceneFlag::None);
                }
                me->DespawnOrUnsummon(15s);
            }
        }
