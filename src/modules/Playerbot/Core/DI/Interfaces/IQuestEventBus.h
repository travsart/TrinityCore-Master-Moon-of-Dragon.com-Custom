...
/**
 * @brief Interface for Quest Event Bus
 *
 * Central event distribution system for all quest-related events.
 * Implements thread-safe, priority-based event bus for quest
 * acceptance, completion, and objective tracking.
 *
 * Features:
 ...
 */
class TC_GAME_API IQuestEventBus
{
public:
    virtual ~IQuestEventBus() = default;
    ...
    /**
     * @brief Publish a quest event to all subscribers
     * @param event The event to publish
     * @return true if event was queued successfully
     * @note Thread-safe: Can be called from any thread
     */
    virtual bool PublishEvent(QuestEvent const& event) = 0;
    ...
    /**
     * @brief Subscribe to specific quest event types
     ...
     */
    virtual bool Subscribe(BotAI* subscriber, std::vector<QuestEventType> const& types) = 0;

    /**
     * @brief Subscribe to all quest event types
     ...
     */
    virtual std::vector<QuestEvent> GetQueueSnapshot() const = 0;
};
