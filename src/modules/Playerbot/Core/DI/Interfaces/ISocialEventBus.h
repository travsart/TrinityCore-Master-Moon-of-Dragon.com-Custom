...
/**
 * @brief Interface for Social Event Bus
 *
 * Central event distribution system for all social-related events
 * (chat, emotes, guild invites, trade status).
 *
 * Features:
 ...
 */
class TC_GAME_API ISocialEventBus
{
public:
    virtual ~ISocialEventBus() = default;
    ...
    /**
     * @brief Publish a social event to all subscribers
     * @param event The event to publish
     * @return true if event was queued successfully
     * @note Thread-safe: Can be called from any thread
     */
    virtual bool PublishEvent(SocialEvent const& event) = 0;
    ...
    /**
     * @brief Subscribe to specific social event types
     ...
     */
    virtual bool Subscribe(BotAI* subscriber, std::vector<SocialEventType> const& types) = 0;

    /**
     * @brief Subscribe to all social event types
     ...
     */
    virtual std::vector<SocialEvent> GetQueueSnapshot() const = 0;
};
