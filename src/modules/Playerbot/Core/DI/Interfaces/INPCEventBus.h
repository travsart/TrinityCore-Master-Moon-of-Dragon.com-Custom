...
/**
 * @brief Interface for NPC Event Bus
 *
 * Central event distribution system for all NPC interaction events.
 * Implements thread-safe event bus for gossip menus, vendors, trainers,
 * banks, spirit healers, and petition interactions.
 *
 * Features:
 ...
 */
class TC_GAME_API INPCEventBus
{
public:
    ...
    /**
     * @brief Publish an NPC event to all subscribers
     * @param event The event to publish
     * @return true if event was published successfully
     * @note Thread-safe: Can be called from any thread
     */
    virtual bool PublishEvent(NPCEvent const& event) = 0;
    ...
    /**
     * @brief Subscribe BotAI to specific NPC event types
     ...
     */
    virtual uint64 GetEventCount(NPCEventType type) const = 0;
};
