...
/**
 * @brief Interface for Loot Event Bus
 *
 * Central event distribution system for all loot-related events.
 * Implements thread-safe, priority-based event bus for loot
 * window operations, loot rolls, and item distribution.
 *
 * Features:
 ...
 */
class TC_GAME_API ILootEventBus
{
public:
    virtual ~ILootEventBus() = default;
    ...
    /**
     * @brief Publish a loot event to all subscribers
     * @param event The event to publish
     * @return true if event was queued successfully
     * @note Thread-safe: Can be called from any thread
     */
    virtual bool PublishEvent(LootEvent const& event) = 0;
    ...
    /**
     * @brief Subscribe to specific loot event types
     ...
     */
    virtual bool Subscribe(BotAI* subscriber, std::vector<LootEventType> const& types) = 0;

    /**
     * @brief Subscribe to all loot event types
     ...
     */
    virtual std::vector<LootEvent> GetQueueSnapshot() const = 0;
};
