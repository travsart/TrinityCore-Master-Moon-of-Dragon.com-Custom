...
/**
 * @brief Interface for Aura Event Bus
 *
 * Central event distribution system for all aura-related events.
 * Implements thread-safe, priority-based event bus for aura
 * applications, removals, updates, dispels, and modifiers.
 *
 * Features:
 ...
 */
class TC_GAME_API IAuraEventBus
{
public:
    virtual ~IAuraEventBus() = default;
    ...
    /**
     * @brief Publish an aura event to all subscribers
     * @param event The event to publish
     * @return true if event was queued successfully
     * @note Thread-safe: Can be called from any thread
     */
    virtual bool PublishEvent(AuraEvent const& event) = 0;
    ...
    /**
     * @brief Subscribe to specific aura event types
     ...
     */
    virtual bool Subscribe(BotAI* subscriber, ::std::vector<AuraEventType> const& types) = 0;

    /**
     * @brief Subscribe to all aura event types
     ...
     */
    virtual ::std::vector<AuraEvent> GetQueueSnapshot() const = 0;
};
