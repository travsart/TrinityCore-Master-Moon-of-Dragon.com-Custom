...
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM playerbot.playerbot_accounts pa WHERE account_id = NEW.account AND pa.is_bot = 1) THEN
...
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM playerbot.playerbot_accounts pa WHERE account_id = OLD.account AND pa.is_bot = 1) THEN
        UPDATE playerbot.playerbot_accounts 
