...
void MageAI::OnCombatStart(::Unit* target)
{
    ...
    if (_positionManager && target)
        _positionManager->SetOptimalPosition(target, OPTIMAL_CASTING_RANGE);
...
Position MageAI::GetOptimalPosition(::Unit* target)
{
    ...
    if (_positionManager)
        return _positionManager->GetOptimalCastingPosition(target, OPTIMAL_CASTING_RANGE);
...
void MageAI::FindSafeCastingPosition()
{
    ...
    Position safePos = _positionManager->FindSafePosition(OPTIMAL_CASTING_RANGE);
...
::Unit* MageAI::GetBestCounterspellTarget()
{
    ...
    if (_interruptManager)
        return _interruptManager->GetHighestPriorityTarget();
...
bool MageAI::ShouldInterrupt(::Unit* target)
{
    ...
    return _interruptManager && _interruptManager->ShouldInterrupt(target);
}
...
bool MageAI::HasTooMuchThreat()
{
    ...
    return _threatManager->GetThreatPercent() > 80.0f;
}
...
void MageAI::ReduceThreat()
{
    ...
    if (_threatManager && _threatManager->GetThreatPercent() > 95.0f)
    {
        UseIceBlock();
    }
}
...
::Unit* MageAI::SelectOptimalTarget(const std::vector<::Unit*>& enemies)
{
    ...
    if (_targetSelector)
        return _targetSelector->SelectBestTarget(enemies);
...
void MageAI::PredictEnemyMovement(::Unit* target)
{
    ...
    Position predictedPos = _positionManager->PredictPosition(target, 2.0f); // 2 seconds ahead
}
...
uint32 MageSpellCalculator::CalculateFireballDamage(Player* caster, ::Unit* target)
{
    ...
    float spellPower = caster->GetFloatValue(PLAYER_FIELD_MOD_DAMAGE_DONE_POS);
    baseDamage += static_cast<uint32>(spellPower * 1.0f); // 100% spell power coefficient
...
uint32 MageSpellCalculator::CalculateFrostboltDamage(Player* caster, ::Unit* target)
{
    ...
    // Apply spell power scaling
    float spellPower = caster->GetFloatValue(PLAYER_FIELD_MOD_DAMAGE_DONE_POS);
    baseDamage += static_cast<uint32>(spellPower * 0.95f); // 95% spell power coefficient
...
uint32 MageSpellCalculator::CalculateArcaneMissilesDamage(Player* caster, ::Unit* target)
{
    ...
    // Apply spell power scaling
    float spellPower = caster->GetFloatValue(PLAYER_FIELD_MOD_DAMAGE_DONE_POS);
    baseDamage += static_cast<uint32>(spellPower * 1.2f); // 120% spell power coefficient
...
float MageSpellCalculator::GetHasteModifier(Player* caster)
{
    ...
    float hasteRating = caster->GetFloatValue(PLAYER_FIELD_HASTE_RATING);
    float hastePct = hasteRating / 32.79f; // Conversion factor for level 80
...
float MageSpellCalculator::CalculateCritChance(uint32 spellId, Player* caster, ::Unit* target)
{
    ...
    // Base crit chance
    float critChance = caster->GetFloatValue(PLAYER_FIELD_CRIT_PERCENTAGE);
