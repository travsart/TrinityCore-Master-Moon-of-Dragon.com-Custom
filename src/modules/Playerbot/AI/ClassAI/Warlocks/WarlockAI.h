...
// Warlock AI implementation with full pet and DoT management
...
private:
    // Warlock-specific data
    WarlockSpec _specialization;
    ...
    uint32 _petHealthPercent;
    uint32 _lastPetHeal;
    bool _petNeedsHealing;
    bool _petIsActive;
    Position _lastPetPosition;

    // Soul shard management
    SoulShardInfo _soulShards;
    uint32 _lastSoulShardUse;
    bool _needsSoulShards;
    ...
    uint32 _lastFear;
    uint32 _lastBanish;
    uint32 _lastShadowflame;
    uint32 _lastRainOfFire;
    std::unordered_map<ObjectGuid, uint32> _fearTargets;
    std::unordered_map<ObjectGuid, uint32> _banishTargets;

    // Rotation methods by specialization
    void UpdateAfflictionRotation(::Unit* target);
    void UpdateDemonologyRotation(::Unit* target);
    void UpdateDestructionRotation(::Unit* target);
