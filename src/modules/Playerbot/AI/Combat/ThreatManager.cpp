...
ThreatManager::ThreatManager(Player* bot)
...
void ThreatManager::UpdateThreat(uint32 diff)
{
    if (!_bot)
        return;
...
void ThreatManager::ResetThreat()
{
    std::unique_lock<std::shared_mutex> lock(_mutex);
    _threatMap.clear();
...
float ThreatManager::CalculateThreat(Unit* target) const
{
    if (!target || !_bot)
        return 0.0f;
    ...
    auto endTime = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime);
    const_cast<ThreatManager*>(this)->TrackPerformance(duration, "CalculateThreat");

    return finalThreat;
}

float ThreatManager::CalculateThreatPercent(Unit* target) const
{
    if (!target || !_bot)
        return 0.0f;
...
void ThreatManager::UpdateThreatValue(Unit* target, float threat, ThreatType type)
{
    if (!target || !_bot)
        return;
...
void ThreatManager::ModifyThreat(Unit* target, float modifier)
{
    if (!target || !_bot)
        return;
...
ThreatAnalysis ThreatManager::AnalyzeThreatSituation()
{
    auto startTime = std::chrono::high_resolution_clock::now();
    ...
    auto endTime = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime);
    const_cast<ThreatManager*>(this)->TrackPerformance(duration, "AnalyzeThreatSituation");
...
std::vector<ThreatTarget> ThreatManager::GetSortedThreatTargets()
{
    ThreatAnalysis analysis = AnalyzeThreatSituation();
    return analysis.sortedTargets;
}

ThreatTarget* ThreatManager::GetPrimaryThreatTarget()
{
    ThreatAnalysis analysis = AnalyzeThreatSituation();
    return analysis.primaryTarget;
}

ThreatTarget* ThreatManager::GetSecondaryThreatTarget()
{
    ThreatAnalysis analysis = AnalyzeThreatSituation();
    return analysis.secondaryTarget;
}

void ThreatManager::SetTargetPriority(Unit* target, ThreatPriority priority)
{
    if (!target)
        return;
...
ThreatPriority ThreatManager::GetTargetPriority(Unit* target) const
{
    if (!target)
...
void ThreatManager::UpdateTargetPriorities()
{
    std::unique_lock<std::shared_mutex> lock(_mutex);
...
void ThreatManager::UpdateRoleBasedThreat()
{
...
bool ThreatManager::HasThreat(Unit* target) const
{
    if (!target)
        return false;
...
float ThreatManager::GetThreat(Unit* target) const
{
    if (!target)
        return 0.0f;
...
float ThreatManager::GetThreatPercent(Unit* target) const
{
    if (!target)
        return 0.0f;
...
ThreatInfo const* ThreatManager::GetThreatInfo(Unit* target) const
{
    if (!target)
        return nullptr;
...
std::vector<Unit*> ThreatManager::GetAllThreatTargets()
{
...
std::vector<Unit*> ThreatManager::GetThreatTargetsByPriority(ThreatPriority priority)
{
    std::vector<Unit*> targets;
...
uint32 ThreatManager::GetThreatTargetCount() const
{
    std::shared_lock<std::shared_mutex> lock(_mutex);
...
bool ThreatManager::IsInThreatEmergency() const
{
    ThreatAnalysis analysis = const_cast<ThreatManager*>(this)->AnalyzeThreatSituation();
    return analysis.emergencyResponse || analysis.threatOverload;
}

std::vector<Unit*> ThreatManager::GetEmergencyTargets()
{
    return GetThreatTargetsByPriority(ThreatPriority::CRITICAL);
}

void ThreatManager::HandleThreatEmergency()
{
...
// Event handlers
void ThreatManager::OnDamageDealt(Unit* target, uint32 damage)
{
    if (!target || damage == 0)
        return;
...
void ThreatManager::OnHealingDone(Unit* target, uint32 healing)
{
    if (!target || healing == 0)
        return;
...
void ThreatManager::OnSpellInterrupt(Unit* target)
{
    if (!target)
        return;
...
void ThreatManager::OnTauntUsed(Unit* target)
{
    if (!target)
        return;
...
void ThreatManager::OnThreatRedirect(Unit* from, Unit* to, float amount)
{
    if (!from || !to)
        return;
...
float ThreatManager::CalculateBaseThreat(Unit* target) const
{
    if (!target || !_bot)
        return 0.0f;
...
float ThreatManager::CalculateRoleModifier() const
{
    switch (_botRole)
    {
        case ThreatRole::TANK:
...
float ThreatManager::CalculateDistanceModifier(Unit* target) const
{
    if (!target || !_bot)
        return 1.0f;
...
float ThreatManager::CalculateHealthModifier(Unit* target) const
{
    if (!target)
        return 1.0f;
...
float ThreatManager::CalculateAbilityModifier(Unit* target) const
{
    if (!target)
        return 1.0f;
...
void ThreatManager::AnalyzeTargetThreat(Unit* target, ThreatTarget& threatTarget)
{
    if (!target)
        return;
...
void ThreatManager::ClassifyThreatPriority(ThreatTarget& threatTarget)
{
...
void ThreatManager::UpdateThreatHistory(Unit* target, float threat)
{
    if (!target)
        return;
...
void ThreatManager::UpdateThreatTable(uint32 diff)
{
    uint32 now = getMSTime();
...
void ThreatManager::CleanupStaleEntries()
{
    uint32 now = getMSTime();
...
void ThreatManager::UpdateDistances()
{
    for (auto& [guid, info] : _threatMap)
    {
        Unit* target = ObjectAccessor::GetUnit(*_bot, guid);
...
void ThreatManager::UpdateCombatState()
{
    for (auto& [guid, info] : _threatMap)
    {
        Unit* target = ObjectAccessor::GetUnit(*_bot, guid);
...
void ThreatManager::TrackPerformance(std::chrono::microseconds duration, const std::string& operation)
{
