...
// Static object pool for zero-allocation after warmup
thread_local boost::object_pool<WorldPacket> BotSession::s_packetPool;

BotSession::BotSession(uint32 bnetAccountId)
    : WorldSession(
        sAccountMgr->GetAccountIdByBattlenetAccount(bnetAccountId), // Use legacy account ID for Trinity compatibility
        ...
        Minutes(0),                                              // Timezone
        0,                                                       // Build
        ClientBuild::Classic,                                    // Client build
        LOCALE_enUS,                                            // Locale
        0,                                                       // Recruiter
        false),                                                  // Is recruiter
...
void BotSession::QueuePacket(WorldPacket&& packet)
{
    ...
    // Move packet to TBB concurrent queue for lock-free processing
    auto pooledPacket = std::unique_ptr<WorldPacket>(_packets.packetPool.construct(std::move(packet)));

    if (!_packets.incomingQueue.try_push(std::move(pooledPacket))) {
        TC_LOG_WARN("module.playerbot.session",
            "Failed to queue incoming packet for account {}", GetAccountId());
        _packets.droppedPackets.fetch_add(1, std::memory_order_relaxed);
    } else {
...
size_t BotSession::ProcessIncomingPacketBatch()
{
    ...
    for (size_t i = 0; i < _packets.BATCH_SIZE && _packets.processingBatch[i]; ++i) {
        ...
        try {
            // Handle the packet using WorldSession's existing system
            WorldSession::HandleIncomingPacket(*packet);
            ++processed;
        }
...
void BotSession::Hibernate()
{
    ...
    if (!_state.compare_exchange_strong(expected, State::HIBERNATED)) {
...
{
    State expected = State::HIBERNATED;
    if (!_state.compare_exchange_strong(expected, State::ACTIVE)) {
...
void BotSession::ExecuteBotMetadataQuery(PreparedStatement* stmt)
{
    if (!stmt) return;
