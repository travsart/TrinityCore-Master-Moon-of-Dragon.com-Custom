...
CharacterDatabasePreparedStatement* BotSession::GetSafePreparedStatement(CharacterDatabaseStatements statementId, const char* statementName)
{
    ...
    // If a statement fails to prepare, the issue is in database connection initialization
    // Log any failed statement access to help diagnose connection preparation issues
...
bool BotSession::LoginCharacter(ObjectGuid characterGuid)
{
    ...
    try
    {
        ...
        // CRITICAL FIX: CHAR_SEL_CHARACTER is CONNECTION_ASYNC, must use proper async handling
        // But since we need immediate results, this is problematic - need to redesign for async callbacks
...
void BotSession::StartAsyncLogin(ObjectGuid characterGuid)
{
    ...
    try
    {
        ...
        TC_LOG_INFO("module.playerbot.session",
            "🔍 About to execute AsyncQuery for statement on playerbot_characters database");
